...
enum ImageMemoryArea { FlashImageArea = 1, ScreenImageArea = 2 };

enum ScrollMode { WrappingMode = 1, VertScrollMode = 2, HorizScrollMode = 3 };
...
enum ScreenSaver { AllDotsOffSaver = 2, AllDotsOnSaver = 3, InvertSaver = 4 };
...
enum FontFormat { GU70005x7Format = 0, GU70007x8Format = 1, CUUFormat = 2, LCDFormat = CUUFormat };
...
enum Charset {
  CP437 = 0,
  EuroStdCharset = CP437,
  Katakana = 1,
  CP850 = 2,
  MultilingualCharset = CP850,
  CP860 = 3,
  PortugeseCharset = CP860,
  CP863 = 4,
  CanadianFrenchCharset = CP863,
  CP865 = 5,
  NordicCharset = CP865,
  CP1252 = 0x10,
  CP866 = 0x11,
  Cyrillic2Charset = CP866,
  CP852 = 0x12,
  Latin2Charset = CP852,
  CP858 = 0x13
};
...
class Noritake_VFD_GU7000 {
 ...
 public:
  ...
  void GU7000_drawFROMImage(unsigned long address, uint8_t srcHeight, unsigned width,
                            uint8_t height);
  ...
  void GU7000_drawImage(unsigned x, uint8_t y, ImageMemoryArea area, unsigned long address,
                        uint8_t srcHeight, unsigned width, uint8_t height, unsigned offsetx,
                        unsigned offsety);
  void GU7000_drawImage(unsigned x, uint8_t y, ImageMemoryArea area, unsigned long address,
  ...
  void GU7000_drawImage_p(unsigned x, uint8_t y, unsigned width, uint8_t height,
                          const uint8_t *data);
};
