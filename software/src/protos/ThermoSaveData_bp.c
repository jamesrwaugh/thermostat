...
int EncodeThermoSaveData(struct ThermoSaveData *m, unsigned char *s) {
    ...
    s[7] |= (((unsigned char *)&((*m).pad))[0] << 7) & 128;
    s[8] = (((unsigned char *)&((*m).have_mqtt))[0] ) & 255;
    memcpy(&s[9], (*m).mqtt.wifi_name, 33);
    memcpy(&s[42], (*m).mqtt.wifi_password, 65);
    return 0;
}
...
int DecodeThermoSaveData(struct ThermoSaveData *m, unsigned char *s) {
    ...
    ((unsigned char *)&((*m).pad))[0] = (s[7] >> 7) & 1;
    ((unsigned char *)&((*m).have_mqtt))[0] = (s[8] ) & 255;
    memcpy((*m).mqtt.wifi_name, &s[9], 33);
    memcpy((*m).mqtt.wifi_password, &s[42], 65);
    return 0;
}